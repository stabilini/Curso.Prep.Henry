...
function asAmirror(str) {
  ...
  var palabras_invertidas = [];
  for (var n = 0; n < palabras.length; n++) {
    palabras_invertidas[n] = palabras[n].split('').reverse().join('');
  }
  return palabras_invertidas.join(' ');
} 
...
function deleteAbc(cadena){
  ...
  for (var n = 0; n < cadena.length; n++) {
    if (cadena[n] != 'a' && cadena[n] != 'b' && cadena[n] != 'c') {
      temporal += cadena[n];
    }
  }
  return temporal;
